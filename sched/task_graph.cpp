...
{
    out<<"[task: "<<p.task_index<<", exec_time: "<<p.exec_time<<", in_degree: "<<p.in_degree<<", out_degree: "<<p.out_degree<<", deadline: "<<p.deadline<<", type: "<<p.type<<", arc_node: [";
    ArcNode *a=p.next;
    while (a!= nullptr)
    {
        out<<*a;
...
{
    return out<<"(task: "<<a.task_index<<", type: "<<a.type<<", com_time: "<<a.com_time<<")";
}